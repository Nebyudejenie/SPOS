...
- **Merchant dedup** — [etl/dedup_merchants.py](etl/dedup_merchants.py) conservatively merges name-only duplicate merchants into their canonical (coded) record, re-pointing every child row first. Result: **10,519 → 9,166** merchants (1,353 merged), zero orphaned FKs, idempotent on re-run. Distinct coded merchants are never merged into each other.
